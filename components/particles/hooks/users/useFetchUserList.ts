...
export const useFetchUserList = (excludeWebPortalUser?: boolean) => {
  ...
  return useQuery({
    queryKey: [
      'user.list',
      { page, search, content, excludeWebPortalUser, startDate, endDate, lastLogin, numberOfPost },
    ],
    queryFn: ({ signal }) =>
      getUserList({ signal, page, search, content, excludeWebPortalUser, startDate, endDate, lastLogin, numberOfPost }),
  });
};
