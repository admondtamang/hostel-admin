...
const BlogBuilderNavBar: React.FC<IBlogBUilderNavBar> = ({ content, setContent, loading, autoSave = false }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('click', containerNavCheck);

    return () => window.removeEventListener('click', containerNavCheck);
  }, []);
  ...
  return (
    <div className="">
      ...
      <header className={clsx('container', BlogBuilderNavCSS.navContainer)}>
        <Logo logoColor="#EFB10F" textColor="#262626" className={BlogBuilderNavCSS.navLogo} />
